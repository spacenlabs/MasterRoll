...
const JobApplicationModal: React.FC<JobApplicationModalProps> = ({ job, isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await submitForm('Job Application', {
        jobId: job.id,
        ...data,
        cv: cvFile ? cvFile.name : 'No file'
    });
        ...
        {isSuccess ? (
          <div className="p-12 text-center">
            ...
            <p className="text-slate-600 mb-8">
              Your application for <strong>{job.title}</strong> has been submitted. The hiring team will review your profile and contact you via email if shortlisted.
            </p>
            <button 
              onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="full_name" required type="text" className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-brand-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Email</label>
                  <input name="email" required type="email" className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-brand-500 outline-none" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Mobile Number</label>
                <input name="mobile" required type="tel" className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-brand-500 outline-none" />
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Cover Letter (Optional)</label>
                <textarea name="cover_letter" rows={3} className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-brand-500 outline-none" placeholder="Briefly describe why you are a good fit..."></textarea>
              </div>
